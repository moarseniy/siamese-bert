...
Если в одном ответе к одной теме выражены разные отношения, один код можно
указать с несколькими тональностями: `I4:1, I4:2`. Точный дубль одной пары
считается один раз.
...
тональностей. Один код может присутствовать в нескольких парах с разными
тональностями. Если подходящих кодов нет, возвращается `UNKNOWN`.
...
`--after-semicolon-prefix` вставляет заданный текст сразу после первого `;`:

```text
8; зарплата и офис
→ 8; нужно улучшить: зарплата и офис
```

Строки без `;` остаются без изменений. Исходная колонка в выходном файле не
переписывается: преобразованный текст используется только в запросе к LLM.
...
- ответ ограничен JSON Schema
  `{"labels":[{"code":"A1","sentiment":2}]}` с максимумом 6 пар;
- `temperature=0`;
...
Если один код получил разные тональности, он повторяется в позиционных колонках,
например `predicted_codes = I4, I4` и `predicted_sentiments = 1, 2`.
...
`micro_f1` оценивают наличие кодов и считают повторяющийся код один раз.
Метрики `joint_micro_f1`,
`joint_macro_f1` и `gold_code_sentiment_accuracy` дополнительно требуют
все правильные пары кода и тональности.

## Пересчёт метрик

Метрики для уже сохранённого файла можно пересчитать без запросов к vLLM:

```bash
python scripts/evaluate.py \
  --input predictions_new.csv \
  --codebook /path/to/canonical_codebook.csv \
  --gold-codes-col "Коды_новые"
```

Команда обновит файлы `*_stats.json`, `*_per_class.csv` и `*_errors.csv` рядом
с таблицей предсказаний. Это также работает для разметки вида
`I4:1, I4:2`.
