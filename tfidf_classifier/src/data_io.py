...
def parse_codebook(path: str | Path) -> pd.DataFrame:
    ...
    frame["parent_code"] = frame["code"].map(parent_code)
    names = frame.set_index("code")["name"].to_dict()
    frame["parent_name"] = frame["parent_code"].map(names).fillna("")
    frame["is_parent"] = frame["parent_code"] == frame["code"]
...
def load_labeled_data(
    ...
    if invalid_codes:
        preview = ", ".join(sorted(invalid_codes)[:20])
