...
def encode_labels(code_lists: list[list[str]], classes: list[str]) -> np.ndarray:
    ...
    targets = np.zeros((len(code_lists), len(classes)), dtype=np.float32)
    for row_index, codes in enumerate(code_lists):
        for code in codes:
            if code in index_by_code:
                targets[row_index, index_by_code[code]] = 1.0
    return targets
...
def calculate_metrics(
    ...
    predictions = threshold_predictions(probabilities, threshold, max_labels=max_labels)
